...
Whether some games work depends largely on how they were cracked/unprotected; sometimes it might work simply by using another **.ST** image with a different crack. By the way, **ASE** currently only handles disk images in **.ST** format.
...
It runs on **Windows**, **Mac**, and should also work on **Linux**, although I haven't tested it. On **Windows**, it will work with any **TOS** version up to 1.04. On Mac, only version 1.00 will work due to the way C# handles exceptions on that system; it doesn't get past the Blitter detection during boot. I assume the same will happen on Linux.
